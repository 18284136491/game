...
class SettingController extends AdminbaseController
{
    ...
    public function map()
    {
        $map_settings = sp_get_option('map_settings');
        $this->assign('map_settings',$map_settings);
        $this->display();
    }
    ...
    public function map_post()
    {
        if (IS_POST) {
            ...
            $phone = I('post.phone');
            $intro = I('post.intro');
            $info = file_get_contents('http://api.map.baidu.com/geocoder/v2/?address='.$address.'&output=json&ak='.$ak);
            $ret = json_decode($info,true);
            if(!$ak || !$title || !$address || !$phone || !$intro){
                $this->error('请完整填写信息！');
            }
            // 验证座机号码
            if(!preg_match('/^(0[0-9]{2,3}-)?([2-9][0-9]{6,7})+(-[0-9]{1,4})?$/',$phone)){
                $this->error('座机号码格式不正确！');
            }
            if($ret['status'] == 1){
                $this->error('无相关结果！');
            }
            ...
            $result = sp_set_option('map_settings', $data);
            if ($result !== false) {
                $this->success('保存成功！');
            } else {
                $this->error('保存失败！');
